...
public class Differentiation extends Activity implements OnClickListener{

	Button calculate,graph,reset;
	EditText att,function;
	TextView ans;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		att=(EditText)findViewById(R.id.a1a1);
	...
	public void onClick(View ae) 
	{
		...
		{
			double at1=Double.parseDouble(att.getText().toString());
            double dx=0.001, yintercept;
            ...
            functionstr1=changetoqformat(functionstr);//changing to q format
           	...
           	slope=(float) ((function(functionstr1,at1+dx)-function(functionstr1,at1))/dx);
           	yintercept=function(functionstr1,at1)-slope*at1;
           	
           	if(slope<0.0001&&slope>-0.0001){slope=0;}
		...
		else if(str.equals("graph"))
		{
	    	...
	    	graph.putExtra("fromwhere","differentiate");
	    	graph.putExtra("at",att.getText().toString());
	    	
	    	graph.putExtra("slope",ans.getText().toString());
	    	
	    	graph.putExtra("x0",x0+"");
	    	graph.putExtra("y0",y0+"");
	    	graph.putExtra("x1",x1+"");
	    	graph.putExtra("y1",y1+"");
